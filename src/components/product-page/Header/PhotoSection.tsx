    ...
    <div className="flex flex-col-reverse lg:flex-row lg:space-x-3.5">
      ...
      {data?.gallery && data.gallery.length > 0 && (
        <div className="flex lg:flex-col space-x-3 lg:space-x-0 lg:space-y-3.5 w-full lg:w-fit items-center lg:justify-start justify-center">
          ...
          {/* Map through gallery images and display them */}
          {data.gallery.map((photo, index) => (
            <button
              ...
              onClick={() => setSelected(photo)} // Set the selected image on click
            >
              {/* Image thumbnail */}
              <Image
                src={photo}
                width={152}
                height={167}
                className="rounded-md w-full h-full object-cover" // Removed hover and transition effects
                alt={data.title}
                priority
              />
            </button>
